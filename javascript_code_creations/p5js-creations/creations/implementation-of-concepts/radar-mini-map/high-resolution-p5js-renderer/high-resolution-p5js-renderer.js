...
//console.log(Math.cos(this.polarCord.theta))//* radiansToDegrees)
    ...
    renderGraphic.circle(this.display.x,this.display.y,10*worldZoom);
  }
}
...
class character{
  ...
  radar(obstaclesIn) {
    ...
    for (let i=1 ; i <= this.radarRadius/this.radarInterval;i++) {
      ...
      renderGraphic.circle(this.radarLoc.x,this.radarLoc.y,i*this.radarInterval*2)
    }
    this.radarDisplay = [];
    for (let i=0; i < obstaclesIn.length;i++) {
      renderGraphic.stroke(255);
  ...
  lightLance() {
    if(this.currentLightLance) {
      ...
      renderGraphic.stroke(255) 
      renderGraphic.line(this.locationOfSelectedLightLance.x ,this.locationOfSelectedLightLance.y,200,200)
    }
  }
}
...
function exportHighRes() {
    ...
    save(renderGraphic, "radar-mini-map-render", 'png');
